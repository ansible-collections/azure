...
stream_declarations_spec_columns_options = dict(
    name=dict(type='str'),
    type=dict(type='str', choices=['string', 'int', 'long', 'real', 'boolean', 'datetime', 'dynamic'])
)

stream_declarations_spec = dict(
    columns=dict(type='list', elements='dict', options=stream_declarations_spec_columns_options)
)
...
class AzureRMDataCollectionRules(AzureRMModuleBaseExt):
    ...
    def __init__(self):
        self.module_arg_spec = dict(
            ...
            stream_declarations=dict(type='dict', options=stream_declarations_spec),
    ...
    def exec_module(self, **kwargs):
        ...
        if self.state == 'present':
            if before_dict is None:
                ...
                update_tags, data_collection_rule_update['tags'] = self.update_tags(before_dict.get('tags'))
                if update_tags:
                    self.results['changed'] = True
                    if self.check_mode:
                        # Check mode, skipping actual creation
                ...
                elif not self.default_compare({}, data_collection_rule_update, before_dict, '', result_compare):
