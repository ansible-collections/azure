...
options:
    ...
    include_arc_resource_groups:
        description:
            - A list of resource group names to search for Azure ARC machines. '\*' will include all
              resource groups in the subscription.
        default: []
    include_vmss_resource_groups:
        description:
            - A list of resource group names to search for virtual machine scale sets (VMSSs). '\*' will
              include all resource groups in the subscription.
        default: []
    ...
    keyed_groups:
        description:
            - Creates groups based on the value of a host variable. Requires a list of dictionaries,
    ...
    conditional_groups:
        description:
            - A mapping of group names to Jinja2 expressions. When the mapped expression is true, the host
              is added to the named group.
    hostvar_expressions:
        description:
            - A mapping of hostvar names to Jinja2 expressions. The value for each host is the result of the
              Jinja2 expression (which may refer to any of the host's existing variables at the time this inventory
              plugin runs).
    exclude_host_filters:
        description:
            - Excludes hosts from the inventory with a list of Jinja2 conditional expressions. Each
